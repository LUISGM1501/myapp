...
const TareasProAd = () => {
      ...
      try {
          ...
          const tareas = await Promise.all(
              response.data.tareas.map(async (tarea) => {
                  const responsableResponse = await axios.get(`http://192.168.0.17:4000/api/colaborador/${tarea.responsable}`);
                  return {
                      nombre: tarea.nombre,
  ...
  const handleAddTask = async () => {
    try {
      const response = await axios.put(`http://192.168.0.17:4000/api/proyecto/${proyecto._id}/add-task`, {
        nombre: newTaskName,
